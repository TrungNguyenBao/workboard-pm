...
async def seed_pms_tasks(session: AsyncSession, ctx: dict) -> dict:
    ...
    # --- Agile Software Dev (Agile) ---
    wr1 = await _ins(session, ctx, proj="Agile Software Dev", sec="To Do",
    ...
    wr2 = await _ins(session, ctx, proj="Agile Software Dev", sec="To Do",
    ...
    wr3 = await _ins(session, ctx, proj="Agile Software Dev", sec="In Progress",
    ...
    wr4 = await _ins(session, ctx, proj="Agile Software Dev", sec="In Progress",
    ...
    await _ins(session, ctx, proj="Agile Software Dev", sec="Done",
    ...
    # Subtasks for wr2
    await _ins(session, ctx, proj="Agile Software Dev", sec="To Do",
    ...
    # --- Social Media Kanban (Kanban) ---
    await _ins(session, ctx, proj="Social Media Kanban", sec="To Do",
    ...
    mob3 = await _ins(session, ctx, proj="Social Media Kanban", sec="In Progress",
    ...
    mob4 = await _ins(session, ctx, proj="Social Media Kanban", sec="In Progress",
    ...
    await _ins(session, ctx, proj="Social Media Kanban", sec="Done",
        title="Write technical specification doc", status="completed",
        assignee=d, due=now - days(20))
    # Subtask for mob3
    await _ins(session, ctx, proj="Social Media Kanban", sec="In Progress",
        title="Configure APNs certificates for iOS", pri="high", parent=mob3)

    # --- Office Maintenance (Basic) ---
    mkt1 = await _ins(session, ctx, proj="Office Maintenance", sec="To Do",
    ...
    await _ins(session, ctx, proj="Office Maintenance", sec="To Do",
        title="Write Q1 product launch blog post",
        pri="high", assignee=a, due=now + days(20), pos=131072.0)
    mkt3 = await _ins(session, ctx, proj="Office Maintenance", sec="In Progress",
    ...
    await _ins(session, ctx, proj="Office Maintenance", sec="Done",
