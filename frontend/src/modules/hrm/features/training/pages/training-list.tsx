...
const PROGRAM_STATUS_VARIANT: Record<string, string> = {
  planned: 'info',
  active: 'success',
  completed: 'secondary',
  cancelled: 'danger',
...
const ENROLLMENT_STATUS_VARIANT: Record<string, string> = {
  enrolled: 'info',
  in_progress: 'warning',
  completed: 'success',
  dropped: 'secondary',
...
export default function TrainingListPage() {
  ...
  const programColumns: SimpleColumn<TrainingProgram>[] = [
    ...
    { key: 'status', label: 'Status', className: 'w-28', render: (p) => (
      <Badge variant={(PROGRAM_STATUS_VARIANT[p.status] ?? 'secondary') as any}>{p.status}</Badge>
    )},
    ...
    { key: 'actions', label: '', className: 'w-16', render: (p) => (
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100">
        <button className="p-1 text-muted-foreground hover:text-foreground" onClick={(e) => { e.stopPropagation(); setEditProgram(p); setProgramDialogOpen(true) }}>
          <Pencil className="h-3.5 w-3.5" />
        </button>
        <button className="p-1 text-muted-foreground hover:text-destructive" onClick={async (e) => {
          e.stopPropagation()
  ...
  const enrollmentColumns: SimpleColumn<TrainingEnrollment>[] = [
    ...
    { key: 'status', label: 'Status', className: 'w-28', render: (e) => (
      <Badge variant={(ENROLLMENT_STATUS_VARIANT[e.status] ?? 'secondary') as any}>{e.status}</Badge>
    )},
    ...
    { key: 'actions', label: '', className: 'w-20', render: (e) => (
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100">
        {e.status !== 'completed' && (
          <button className="p-1 text-muted-foreground hover:text-emerald-600" title="Mark complete"
            onClick={(ev) => { ev.stopPropagation(); setCompletingEnrollmentId(e.id); setCompletionDialogOpen(true) }}>
            <CheckCircle className="h-3.5 w-3.5" />
          </button>
        )}
        <button className="p-1 text-muted-foreground hover:text-destructive" onClick={async (ev) => {
          ev.stopPropagation()
